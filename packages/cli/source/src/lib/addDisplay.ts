...
export class AddDisplay {
  ...
  public printSummary() {
    ...
    if (failed > 0) {
      ux.log(`${formatCount(failed)} ${pluralized(failed)} failed to be sent`);
    }

    if (remaining && remaining > 0) {
      ux.log(`${formatCount(remaining)} unprocessed ${pluralized(remaining)}`);
    }
